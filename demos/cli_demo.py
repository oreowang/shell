...
# its original forms to accommodate Shell.
...
DEFAULT_CKPT_PATH = 'WisdomShell/Shell-7B-Chat'

_WELCOME_MSG = '''\
Welcome to use Shell-Chat model, type text to start chat, type :h to show command help.
(欢迎使用 Shell-Chat 模型，输入内容即可进行对话，:h 显示命令帮助。)

Note: This demo is governed by the original license of Shell.
...
(注：本演示受Shell的许可协议限制。我们强烈建议，用户不应传播及不应允许他人传播以下内容，包括但不限于仇恨言论、暴力、色情、欺诈相关的有害信息。)
'''
_HELP_MSG = '''\
Commands:
...
def main():
    parser = argparse.ArgumentParser(
        description='Shell-Chat command-line interactive chat demo.')
